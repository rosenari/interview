...
- 자바스크립트는 인터프리터에 의해 코드 한줄마다 바이트코드로 번역하여 실행하는 인터프리터 언어입니다. (한줄한줄 번역하면서 실행하기때문에 컴파일언어보다 처리속도가 느리다.)
- 최신 웹 브라우저들은 `JIT컴파일러가 내장`되어 있어 `자주 반복되는 코드(바이트코드와 profiling data수집)에 한해 네이티브 코드 변환을 통한 최적화`를 진행합니다.
